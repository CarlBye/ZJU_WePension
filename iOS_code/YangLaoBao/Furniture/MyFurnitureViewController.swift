//
//  MyFurnitureViewController.swift
//  YangLaoBao
//
...
class MyFurnitureViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {

    var furnitureTable: UITableView!
    var tableData: [NSDictionary] = [NSDictionary]()
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: MyFurnitureTableViewCell = furnitureTable.dequeueReusableCell(withIdentifier: "furnitureCell") as! MyFurnitureTableViewCell
        let item = tableData[indexPath.row]
        
        cell.furnitureName.text = item["furnName"] as? String
        cell.furnitureID.text = item["furnId"] as? String
        if (item["furnState"] as! String == "1") {
            cell.furnitureState.isOn = true
        } else {
            cell.furnitureState.isOn = false
        }
        ...
        case "3":
            ...
            cell.furnitureType.textColor = UIColor.green
            break
        default:
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if (UserDefaults().string(forKey: "isLogged") == "no") {
            ...
            Alamofire.request(url + "/api/furniture/list",
                              method: .post,
                              parameters: params,
                ...
                if (response.result.isSuccess) {
                    ...
                    if ((resultDictionary["IsSuccess"] as! Bool) == true) {
                        ...
                        //创建表视图
                        self.furnitureTable = UITableView(frame: CGRect(x: 0, y: 60, width: self.view.frame.width, height: self.view.frame.height-200) , style: .plain)
                        //设置代理和数据源
                        ...
                        // 将分割线四周边距设为0
                        self.furnitureTable!.separatorInset = .zero
                        // 以边框为参照点设置分割线边距
                        self.furnitureTable!.separatorInsetReference = .fromCellEdges
                        //创建一个重用的单元格
                        self.furnitureTable!.register(UINib(nibName:"MyFurnitureTableViewCell", bundle:nil), forCellReuseIdentifier:"furnitureCell")
                        self.view.addSubview(self.furnitureTable!)
                        
                    } else {
                        let alert = UIAlertController(title: "获取家具信息失败", message: "\(String(describing: resultDictionary["ErrorInfo"]!))", preferredStyle: .alert)
                        let action = UIAlertAction(title: "确定", style: .default, handler: nil)
                        alert.addAction(action)
                        self.present(alert, animated: true, completion: nil)
                    }
                } else {
                    let alert = UIAlertController(title: "获取家具信息失败", message: "网络错误", preferredStyle: .alert)
                    let action = UIAlertAction(title: "确定", style: .default, handler: nil)
                    alert.addAction(action)
                    self.present(alert, animated: true, completion: nil)
                }
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "我的家具"
